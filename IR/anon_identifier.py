...
data = pd.read_csv('norway_geolocated_id.csv', parse_dates = True)# , usecols = [''])
...
def anonymizer():
    ...
    with open('norway_geolocated_id_translator.json', 'w') as outfile_ID:
        json.dump(ID, outfile_ID) #writing the translation dictionary to file

    data.to_csv('norway_geolocated_id_anonymous.csv', index=False)
