...
places_longlat = pd.read_csv('data/places_ireland_longlat.csv', usecols = ['places','latitude', 'longitude'], index_col = False)
...
def geolocate(user_input):
    ...
    if best_long_elem == "drop":
    	best_long_elem = 0

    if best_lat_elem == "drop":
    	best_lat_elem = 0
...
data = pd.read_csv('data/ireland_output_id.csv',
...
data.to_csv('ireland_geolocated_id.csv')
