...
first_day = datetime.datetime(2010, 1, 1, 0, 0, 0)
last_day = datetime.datetime(2016, 6, 19, 0, 0, 0)
...
for day in date_list:
    ...
    ed = day + datetime.timedelta(days=1)
    end___time = ed.strftime("%Y-%m-%dT%H:%M:%Sz")

#for day in pd.date_range(first_day, last_day, freq='D'):
#    start_time = day.strftime("%Y-%m-%dT%H:%M:%Sz")
#    end___time = (day + pd.Timedelta(1,unit='D')).strftime("%Y-%m-%dT%H:%M:%Sz")

#start_time = "2023-01-01T00:00:00z"
#end___time = "2023-04-11T00:00:00z"
    print(start_time)
    print(end___time)
    ...
    config = {
      ...
      "params": {
        ...
        "end_time": end___time,
        "query": "(wind power OR wind mill OR wind mills OR wind turbines OR wind energy OR offshore wind OR wind park OR wind farm) lang:en",
      ...
      "write_path": "all_data_all_time_EN.txt"
    }
    ...
    def get_formatted_tweets(json_response):
        ...
        print('Length of list of tweets ',len(list_of_tweets))
    ...
    def connect_to_endpoint(url, headers, params):
        ...
        if remaining_requests == 1:
            buffer_wait_time = 15
            resume_time = datetime.datetime.fromtimestamp( int(response.headers["x-rate-limit-reset"]) + buffer_wait_time )
